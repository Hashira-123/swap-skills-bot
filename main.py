...
async def find_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    await update.message.reply_text("🔍 Ищу тебе пару для обмена...")
    ...
    try:
        ...
        response = requests.get(
            ...
            headers={"apikey": SUPABASE_KEY},
            params={"select": "telegram_id, name, username, share, learn, location"}
        )
        if response.status_code != 200 or not response.json():
            await update.message.reply_text("❌ Не удалось загрузить ваш профиль.")
            return

        current_user = response.json()[0]

        # Проверяем, заполнены ли ключевые поля
        if not current_user.get('learn'):
            await update.message.reply_text("❌ Сначала расскажи, чему хочешь научиться. Нажми /profile")
            return
        if not current_user.get('share'):
            await update.message.reply_text("❌ Сначала расскажи, что ты можешь поделиться. Нажми /profile")
            return
        ...
        if all_response.status_code != 200:
            await update.message.reply_text("❌ Не удалось загрузить пользователей.")
            return

        all_users = all_response.json()

        # Фильтруем кандидатов: двусторонний обмен + не в истории
        candidates = [
            u for u in all_users
            if u['telegram_id'] != user.id
            and u['share'] and u['learn']
            and current_user['learn'].lower() in u['share'].lower()
            and current_user['share'].lower() in u['learn'].lower()
        ]

        if not candidates:
            await update.message.reply_text("😔 Пока нет подходящей пары. Попробуй позже.")
            return

        # Берём первого (или random.choice(candidates))
        partner = candidates[0]

        # ✅ УСПЕШНО НАЙДЕНА ПАРА — отправляем результат
        text = (
            f"🎉 Отличные новости!\n\n"
            f"Я нашёл(а) тебе идеальную пару:\n"
            f"👤 {partner['name']}\n"
            f"🔧 Умеет: {partner['share']}\n"
            f"🎯 Хочет учить: {partner['learn']}\n"
            f"📍 Место: {partner['location']}\n\n"
            f"💬 Ты учишь его/её → он/она учит тебя\n\n"
            f"Напиши ему(ей): @{partner['username'] or 'нет'}\n\n"
            f"Договоритесь о времени и формате!"
        )
        await update.message.reply_text(text)

        # 🔽 А ТЕПЕРЬ — попробуем обновить базу (например, отметить notified)
        try:
            requests.patch(
                f"{SUPABASE_URL}/rest/v1/users?telegram_id=eq.{user.id}",
                headers={
            ...
            print("✅ Поле 'notified' успешно обновлено")
        except Exception as patch_error:
            print(f"⚠️ Не удалось обновить 'notified': {patch_error}")
            # ❌ НЕ отправляем сообщение пользователю!
            # Это не критично — пусть думает, что всё ок

    except Exception as main_error:
        # Эта ошибка — если сломался сам поиск (до отправки пары)
        print(f"❌ КРИТИЧЕСКАЯ ошибка поиска: {main_error}")
        await update.message.reply_text("❌ Произошла ошибка при поиске. Попробуй позже.")
