...
async def find_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    await update.message.reply_text("🔍 Ищу тебе пару для взаимного обмена...")

    try:
        # Получаем всех из Supabase
        response = requests.get(
        ...
        # Ищем пару: кто умеет то, чему ты хочешь учиться, и хочет то, что умеешь ты
        candidates = [
            u for u in users if u['telegram_id'] != my_id and u['share']
            and u['learn'] and my_profile['learn'].lower() in
            u['share'].lower()  # Он умеет то, чему ты хочешь учиться
            and my_profile['share'].lower() in
            u['learn'].lower()  # Он хочет то, что умеешь ты
            and (u['location'].lower() in my_profile['location'].lower()
                 or my_profile['location'].lower() in u['location'].lower()
                 )  # Совпадение места
        ]
        ...
        if candidates:
            ...
            await update.message.reply_text(
                f"🎉 Отличные новости!\n\n"
                f"Я нашёл(а) тебе идеальную пару для обмена:\n"
                f"👤 {c['name']}\n"
                f"🔧 Умеет: {c['share']}\n"
                f"🎯 Хочет учить: {c['learn']}\n"
                f"📍 Место: {c['location']}\n\n"
                f"💬 Ты учишь: {my_profile['share']} → он учится у тебя\n"
                f"💬 Он учит: {c['share']} → ты учишься у него\n\n"
                f"Напиши ему(ей): @{c['username'] or 'нет'}\n\n"
                f"Договоритесь о времени и формате обмена!")
        else:
            await update.message.reply_text(
                "Пока нет подходящей пары 😔\n"
                "Но я продолжу искать! Попробуй позже или измени профиль.")
