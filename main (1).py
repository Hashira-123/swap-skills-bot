...
async def find_match(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Проверка: заполнены ли ключевые поля?
        if not current_user.get('learn'):
            await update.message.reply_text(
                "❌ Сначала расскажи, чему хочешь научиться. Нажми /profile")
            return
        if not current_user.get('share'):
            await update.message.reply_text(
                "❌ Сначала расскажи, что ты можешь поделиться. Нажми /profile")
            return

        # 2. Загружаем историю встреч
        matches = current_user.get("matches") or []

        # 3. Получаем всех пользователей из базы
        ...
        # 4. Фильтруем кандидатов:
        # - Это не ты
        # - У него есть навык, которому ты хочешь научиться
        # - Он хочет то, что ты умеешь
        ...
        # 5. Если нет подходящих — завершаем
        if not candidates:
            await update.message.reply_text(
                "😔 Пока нет подходящей пары.\n"
                "Попробуй позже или расширь круг интересов.")
            return

        # 6. Берём случайного кандидата (честнее, чем первый)
        partner = random.choice(candidates)

        # 7. Отправляем результат
        text = (f"🎉 Отличные новости! Я нашёл(а) тебе идеальную пару:\n\n"
                f"👤 <b>{partner['name']}</b>\n"
        ...
        # 8. Сохраняем встречу в истории (обоих пользователей)
        headers = {"apikey": SUPABASE_KEY, "Content-Type": "application/json"}

        # Добавляем партнёра в историю текущего пользователя
        ...
        # Добавляем текущего пользователя в историю партнёра (опционально)
